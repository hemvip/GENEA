	...
	async fetch(request, env, ctx) {
		...
		try {
			...
			if (path === "/api/study/finish" && method === "POST") {
				...
				const { prolificid, studyid, sessionid, actions, screenActions, studySelections, code } = await request.json()

				const { errors, success, data, msg } = await finishStudy(
					client,
					prolificid,
					studyid,
					sessionid,
			...
			if (path === "/api/attention-check" && method === "POST") {
				...
				const { errors, success, data, msg } = await updateAttentionCheck(client, prolificid, studyid, sessionid)
